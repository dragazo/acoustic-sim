...
if __name__ == '__main__':
    ...
    seed = args.seed
    # Record event embeddings
    print('Recording event embeddings...')

    subprocess.check_call([
        'time', 'python', 'sim3.py', '--quiet',
        '--record',
        '--events', *EVENTS,
        ...
        '--event-prob', '1.0',
        '--background-scale', '0.0',
        '--seed', str(seed),
        '--filter', args.filter,
        '--cluster_method', args.cluster_method,
        '--event-freqs', *EVENT_FREQS[0][1:],
    ...
    pbar = tqdm.tqdm(total=queue_init_size, desc='Grid search tasks', unit='task')
    ...
    def worker():
        while True:
            ...
            with open(file, 'w') as f:
                subprocess.check_call([
                    'time', 'python', 'sim3.py', '--quiet',
                    '--replay', # This flag is used to replay the recorded embeddings
                    '--events', *EVENTS,
                    '--backgrounds', *BACKGROUNDS,
                    '--clips', str(round(2 * 60 * args.hours)),
                    '--iterations', str(args.iterations),
